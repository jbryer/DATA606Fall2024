...
countries = read.csv('course_data/WDI_GDF_Country.csv', strip.white=TRUE)
worldData = read.csv('course_data/WDI_GDF_Data.csv', strip.white=TRUE)
...
p + geom_point(aes(size=Population, colour=Region), stat='identity', alpha=.6) + 
	geom_text(hjust=-.2, vjust=.5, size=2) + 
	scale_size_continuous('Population (Millions)') + 
